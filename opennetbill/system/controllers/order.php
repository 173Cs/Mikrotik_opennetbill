...
if(empty(_post('phonenumber'))){
    ...
    $user = ORM::for_table('tbl_customers')->where('username', $name)->find_one();
    if (!$user) {
        $d = ORM::for_table('tbl_customers')->create();
        $d->username = alphanumeric($name, "+_.");
        $d->password = '1';
        $d->fullname = '';
        $d->address =sprintf("%06d",mt_rand(1,999999));;
        $d->email = '';
        $d->phonenumber = (strlen($name) < 14) ? $name : 'invalid';
        $d->service_type='Hotspot';
        $d->save();
        $user = ORM::for_table('tbl_customers')->where('username', $name)->find_one($d->id());
    }
    $ui->assign('_user', $user);
}
    ...
    case 'buy':
        ...
        if ($d) {
            if ($d['pg_url_payment']) {
                ...
                $d->pg_paid_response = '{}'; // Set payment response to empty
                $d->status = 4; // Set transaction status to cancelled
                $d->paid_date = date('Y-m-d H:i:s'); // Set paid date to current date
                $d->save(); // Save changes to the database
                ...
                // Redirect the user to create a new order
                // r2(U . "order/package", 'w', Lang::T("You can create a new order now."));
            } else {
                if ($gateway == $d['gateway']) {
                    $id = $d['id'];
                } else {
        ...
        if (empty ($id)) {
            ...
            $d->payment_channel = 'Mpesa StkPush';
        ...
        if (!$id) {
            if($user['service_type'] == "Hotspot") {
                r2(U . "login", 'e', Lang::T("Failed to create Transaction.."));
            }else{
