...
switch ($action) {
    ...
    case 'activation':
        ...
        break;
    case 'daily-hotspot-report':
        $paginator = Paginator::build(ORM::for_table('tbl_transactions')->where('recharged_on' ,$mdate)->where('type','Hotspot'));
        $d = ORM::for_table('tbl_transactions')->where('type','Hotspot')->where('recharged_on', $mdate)->offset($paginator['startpoint'])->limit($paginator['limit'])->order_by_desc('id')->find_many();
        $dr = ORM::for_table('tbl_transactions')->where('type','Hotspot')->where('recharged_on', $mdate)->sum('price');
        ...
        break;
    case 'daily-pppoe-report':
        $paginator = Paginator::build(ORM::for_table('tbl_transactions')->where('recharged_on' ,$mdate)->where('type','PPPoE'));
        $d = ORM::for_table('tbl_transactions')->where('type','PPPoE')->where('recharged_on', $mdate)->offset($paginator['startpoint'])->limit($paginator['limit'])->order_by_desc('id')->find_many();
        $dr = ORM::for_table('tbl_transactions')->where('type','PPPoE')->where('recharged_on', $mdate)->sum('price');
    ...
    case 'by-period-Hotspot':
        ...
        break;
    case 'by-period-PPPOE':
        $ui->assign('mdate', $mdate);
        $ui->assign('mtime', $mtime);
        $ui->assign('tdate', $tdate);
        run_hook('view_reports_by_period'); #HOOK
        $ui->display('reports-period-pppoe.tpl');
        break;
    case 'period-view':
        $fdate = _post('fdate');
        $tdate = _post('tdate');
        ...
        if ($stype == "Hotspot") {
            $d = ORM::for_table('tbl_transactions')->where('type', $stype);
        }else if ($stype == "PPPoE") {
            $d = ORM::for_table('tbl_transactions')->where('type', $stype);
        }
